...
class GRUED(Algorithm, PyTorchUtils):
    def __init__(self, name: str = 'GRU-ED', num_epochs: int = 20, batch_size: int = 32, lr: float = 1e-4,
                 hidden_size: int = 5, sequence_length: int = 208, train_gaussian_percentage: float = 0.20,
    ...
    def loss_d(self, prototype, d_min=1.0):
        sum = torch.tensor(0)
    ...
    def fit(self, X: pd.DataFrame):
        ...
        #torch.nn.utils.clip_grad_norm_(self.GRUed.parameters(), 5, norm_type=2.0, error_if_nonfinite=False)
        self.GRUed.train()
        ...
        for epoch in trange(self.num_epochs):
            ...
            for ts_batch in train_loader:
                output, enc_hidden, a = self.GRUed(self.to_var(ts_batch), return_latent=True)
                loss_c = self.loss_c(self.GRUed.prototype_layer.prototype, enc_hidden)
                loss_d = self.loss_d(self.GRUed.prototype_layer.prototype, d_min=1.0)
                loss_e = self.loss_e(self.GRUed.prototype_layer.prototype, enc_hidden)
                # loss_w = torch.sum(torch.abs(self.GRUed.hidden2output.weight))
        ...
        a_min = torch.ones(self.GRUed.prototype_layer.k)
        self.proto_input_space_ind = torch.Tensor(self.GRUed.prototype_layer.k, 2)

        for i in range(len(sequences)):
            output, enc_hidden, a = self.GRUed(self.to_var(torch.Tensor(sequences[i]).expand(1, sequences[i].shape[0], sequences[i].shape[1])), return_latent=True)
        ...
        for k in range(self.GRUed.prototype_layer.prototype.shape[0]):
            self.hidden_and_prototype_as_df.loc[len(self.hidden_and_prototype_as_df)] = [
                self.GRUed.prototype_layer.prototype[k].detach().numpy().tolist(), 1]
...
class prototype_layer(nn.Module, PyTorchUtils):
    def __init__(self, hidden_size: int, seed: int, gpu: int, k=2):
        ...
        self.init_values = nn.init.uniform(self.prototype_size, a=-0.5, b=0.5)
        self.prototype = nn.Parameter(self.init_values)
        self.similarity2output = nn.Linear(self.k, 2)

    def forward(self, x, batch_size):
        #initialize a (similarity measure) with 0
        a = torch.zeros((batch_size, self.k))
        #take distance between (latent) decoder and prototype
        d = x.unsqueeze(1) - self.prototype.unsqueeze(1)
...
class prototype_layer(nn.Module, PyTorchUtils):
        ...
        def __init__(self, k, hidden_size):
            ...
            weights = nn.init.uniform_(weights, -0.5, 0.5)
            self.weights = nn.Parameter(weights,  requires_grad=True)  # nn.Parameter is a Tensor that's a module parameter.

        def forward(self, x):
            diff = torch.unsqueeze(x, 1) - self.weights  # 32,16,1 - 30,16 = 32,30,16

            d = torch.norm(diff, dim=2)  # 32,30
            return torch.exp(-d)
"""
...
class GRUEDModule(nn.Module, PyTorchUtils):
    def __init__(self, n_features: int, hidden_size: int,
        ...
        #batch_first- if true, i/p & o/p tensors are provided as (batch,seq,feature) instead (seq,b,f)
    ...
    def forward(self, ts_batch, return_latent: bool = False):
        ...
        return (output, enc_hidden[-1], a) if return_latent else output
